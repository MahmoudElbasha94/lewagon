...
def get_courses(request):
    try:
        ...
        for course in courses:
            if not course.instructor or not getattr(course.instructor, 'user', None):
                print(f"Skipping course id={course.id} due to missing instructor or user.")
                skipped += 1
                continue
            try:
                instructor_name = course.instructor.user.get_full_name()
            except Exception as e:
                print(f"Error getting instructor name for course id={course.id}: {e}")
                skipped += 1
                continue
            courses_data.append({
                'id': course.id,
        ...
        print(f"Total courses: {len(courses)}, Skipped: {skipped}, Returned: {len(courses_data)}")
...
class StudentProfileView(APIView):
    ...
    def put(self, request):
        try:
            ...
            if user_data:
                user_serializer = UserUpdateSerializer(student.user, data=user_data, partial=True)
