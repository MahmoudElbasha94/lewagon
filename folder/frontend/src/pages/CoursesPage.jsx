...
function CoursesPage() {
    ...
    const fetchCourses = async () => {
      try {
        ...
        const [coursesResponse, recommendationsResponse] = await Promise.all([
          axios.get(`http://127.0.0.1:8000/courses/api/courses/?${params.toString()}`, { headers }),
          axios.get('http://127.0.0.1:8000/courses/api/recommendations/', { headers })
        ]);
        ...
        if (searchTerm) {
          const suggestions = coursesResponse.data?.courses
  ...
  }, [searchTerm, selectedCategory, selectedLevel, sortBy, priceRange, selectedSubCategories]);
  ...
  const categories = [
    ...
    { 
      name: 'Programming', 
      icon: FaCode,
      subCategories: ['Web Development', 'Mobile Development', 'Game Development', 'Data Structures', 'Algorithms']
    },
    { 
      name: 'Design', 
      icon: FaPalette,
      subCategories: ['UI/UX', 'Graphic Design', '3D Modeling', 'Animation']
    },
    { 
      name: 'Marketing', 
      icon: FaChartBar,
    ...
    { 
      name: 'Business', 
      icon: FaBriefcase,
      subCategories: ['Entrepreneurship', 'Management', 'Finance', 'Sales']
    },
    { 
      name: 'Data Science', 
      icon: FaDatabase,
      subCategories: ['Machine Learning', 'Data Analysis', 'Big Data', 'Statistics']
    },
  ]
