...
class CourseDetailView(APIView):
    ...
    """
    permission_classes = []  # Allow any user to view course details
    ...
    def get(self, request, slug):
        try:
            ...
            # Calculate progress only for authenticated users
            progress = 0
            is_enrolled = False
            ...
            response_data.update({
                ...
                'lessons': self._prepare_lessons_data(course, request.user)
            })
    ...
    def _prepare_lessons_data(self, course, user):
        """
        Prepare lesson data with completion status for authenticated users
        """
        lessons = []
        for video in course.lessons:
            lesson_data = {
                'id': video.id,
                'title': video.lesson_name,
                'order': video.order,
                'duration': video.duration,
                'video_url': video.video_url if self._can_view_video(user, course) else None,
                'is_completed': self._check_lesson_completion(course, video, user)
            }
            lessons.append(lesson_data)
        return lessons

    def _can_view_video(self, user, course):
        """
        Check if user can view video URLs
        """
        if not user.is_authenticated:
            return False
        if course.courseType.lower() == 'free':
            return True
    ...
    def _check_lesson_completion(self, course, video, user):
        """
        Check lesson completion status for authenticated users
        """
